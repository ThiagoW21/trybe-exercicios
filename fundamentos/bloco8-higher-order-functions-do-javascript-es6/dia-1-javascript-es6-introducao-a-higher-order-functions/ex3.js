...
const verificaAcertos = (gabarito, respostas) => {
  let nota = 0;

  for (let indice = 0; indice < gabarito.length; indice += 1) {
    if (respostas[indice] === gabarito[indice]) {
      nota += 1;
    } else if (respostas[indice] === 'N.A') {
      continue;
    } else {
      nota -= 0.5;
    }
...
console.log(correcao(RIGHT_ANSWERS, STUDENT_ANSWERS, verificaAcertos));
